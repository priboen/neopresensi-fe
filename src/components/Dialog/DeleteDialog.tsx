...
export function DeleteDialog({
  ...
  const handleDelete = async () => {
    if (!entityUuid || !token) return;

    setLoading(true);
    try {
      const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}${endpoint}/${entityUuid}`;
      ...
      if (res.ok) {
        ...
        toast.error(json.message || `Gagal menghapus ${entityName}`);
      }
    } catch (err) {
      toast.error(`Terjadi kesalahan saat menghapus ${entityName}`);
    } finally {
      setLoading(false);
    }
  };
